...
module.exports = function(options) {
  ...
  // Track for all http services published by iOS with name WebDriverAgent
  var browser = bonjour.find({type: 'http'}, function(device) {
    log.info('----------------Tracking iOS devices')
    var webDriverAgentIndex = device.name.indexOf('WebDriverAgent')
    if (device.host && webDriverAgentIndex >= 0) {
      log.info('iOS device: ', device)
      device.id = device.host
      device.type = 'device'
      var deviceName = device.name.substr(webDriverAgentIndex + 'WebDriverAgent-'.length)
      ...
      hash.update(device.id)
      var channel = hash.digest('base64') + '-iOS-' + device.referer.address + '-' + deviceName
      ...
      log.info('iOS device (extended info): ', device)
    }
  })

  browser.on('down', (device) => {
    log.info('----------------down', device)
    push.send([
      wireutil.global
