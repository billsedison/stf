      ...
      socket.on('disconnect', resolve)
        ...
        .on('group.invite', function(channel, responseChannel, data) {
          if(data.platform === 'iOS') {
            ...
            var owner = { email: user.email, name: user.name, group: user.group }
            socket.emit('device.change', {
              important: true
              , data: datautil.applyOwner({
                serial: serial
            ...
            dbapi.setDeviceOwner(serial, owner)
          } else {
            joinChannel(responseChannel) 
            push.send([
              channel
          ...
          if (data.platform === 'iOS') {
            ...
            socket.emit('device.change', {
              ...
              , data: datautil.applyOwner({
                serial: serial
                , owner: null
                , likelyLeaveReason: reason
              }
                , user
              )
            })            
            dbapi.unsetDeviceOwner(serial, owner)
            dbapi.unsetDeviceUsage(serial)
          } else {
            joinChannel(responseChannel)
            push.send([
              channel
